...
`punctuation`（标点转换：内置全角映射 + `"` `'` 成对切换 + 数字后的 `.` 保持半角；`[general] punctuation_map` 的自定义映射（`Engine::set_punctuation_map` 注入）先查、命中即返回，空值 = 原样上屏，`punctuate` 返回 `Option<String>` 支持多字符转换文本，`validate_map` 供壳保存前校验：键单个 ASCII 标点、值 ≤ `MAX_MAP_TEXT_CHARS` 16 字符）/
...
`set_value` 用 toml_edit 原地改键保留注释；`[general] punctuation_map` 行内表（键单个 ASCII 标点、值 ≤ 16 字符，`GeneralConfig::punctuation_map` 读时清洗丢非法条目只警告，`Config::set_punctuation_map` 写前按 Core `validate_map` 拒收）；`[model] enabled` 本地整句模型开关，`LocalModelConfig`；
...
- 配置项：云联想 `[predict]`（偏好设置「云服务」页有「测试连接」按钮：`qingjian_predict::ConnectionTest` 起线程发一条最小请求，`Host` 用独立定时器 `CloudTestMonitor` 轮询结果显示到窗口底部；
  ...
  标点映射 `[general] punctuation_map`（「通用」页「标点映射」行的子弹窗 `pages/punctuation.rs`：行控件即草稿，加 / 删行整表重建并重新分配「删」按钮 tag（`Setting::PunctuationRemoveRow`，tag 基 500），保存时整表校验落盘，空转换文本 = 原样上屏）；
